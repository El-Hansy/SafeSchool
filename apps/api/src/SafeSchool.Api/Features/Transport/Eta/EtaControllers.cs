...
public static class EtaControllers
{
    public static RouteGroupBuilder MapEtaEndpoints(this RouteGroupBuilder group, RouteGroupBuilder guardianGroup)
    {
        ...
        guardianGroup.MapGet("/{studentProfileId}/transport/trips/{tripId:guid}/eta", async (string studentProfileId, Guid tripId, GuardianEtaVisibilityService service, CancellationToken ct) =>
        {
            var result = await service.ReadAsync("school-1", "guardian:me", studentProfileId, tripId, ct);
