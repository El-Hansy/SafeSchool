...
public static class TransportNotificationControllers
{
    public static RouteGroupBuilder MapTransportNotificationEndpoints(this RouteGroupBuilder group, RouteGroupBuilder guardianGroup)
    {
        ...
        guardianGroup.MapGet("/{studentProfileId}/transport/notifications", async (string studentProfileId, GuardianTransportNotificationVisibilityService service, CancellationToken ct) => Results.Ok(await service.ListAsync("school-1", "guardian:me", studentProfileId, ct)));
        return group;
    }
}
