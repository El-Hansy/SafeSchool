...
export async function loadRequestOperations(schoolAccountId = requestsFallbackData.schoolAccountId): Promise<RequestOperationsData> {
  const [board, guardianRequests, studentRequests, outingRequests, earlyLeaveRequests, approvalQueue, history] = await Promise.all([
  ...
  const hasApiData = [board, guardianRequests, studentRequests, outingRequests, earlyLeaveRequests, approvalQueue, history].some((item) => item !== null);
  assertApiDataAvailable("Requests operations", [board, guardianRequests, studentRequests, outingRequests, earlyLeaveRequests, approvalQueue, history], requestsApiBaseUrl());
